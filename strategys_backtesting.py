...
class SnRfollowup(Strategy):
    ...
    def plot_all(self, levels):
        fig, ax = plt.subplots()

        candlestick_ohlc(ax,self.data.df.values,width=0.6, \
        ...
        for level in levels:
            plt.hlines(level[1],xmin=self.data.df['datetime'][level[0]],\
                    xmax=max(self.data.df['datetime']),colors='blue')
        fig.show()
    ...
    def createLevels(self, timeframe=800):
        ...
        # df = pd.DataFrame()
        # df.set_index(self.data.index[-timeframe:])
        # df["open"] = self.data.Open[-timeframe:]
        # df["high"] = self.data.Open[-timeframe:]
        # df["low"] = self.data.Open[-timeframe:]
        # df["close"] = self.data.Open[-timeframe:]

        # df.resample('5m',  how='ohlc', axis=0, fill_method='bfill')
        #candle_len=60
        for i, price in enumerate(self.data.Close[-timeframe:]):
            thisMax = max(self.data.High[-i-10:-i-1])
            thisMin = min(self.data.Low[-i-10:-i-1])
